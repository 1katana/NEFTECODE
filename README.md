...
**Хакатонный прототип мультиагентной системы поддержки оператора нефтеперерабатывающего производства.** Система принимает текущее состояние АВТ и установки гидроочистки 24-2000, оценивает качество продукта и режимный риск, перебирает допустимые сценарии управления и возвращает одно из трёх объяснимых решений: `RECOMMEND`, `KEEP` или `REFUSE`.

Проект работает локально, не требует LLM и может запускаться в закрытом контуре. Текущий статус — **shadow-прототип с защитными блокировками**: модели и полный программный маршрут работают, но автоматическое изменение режима не разрешено без утверждённых технологических границ и проверки причинного отклика установки.
...
Нормальный ответ health для текущей версии — `READY_FOR_SHADOW_WITH_BLOCKERS`. Это означает, что модели загрузились и система готова к теневому расчёту, но операторские воздействия ограничены защитной политикой.
...
4. генерирует ограниченное множество вариантов управления;
5. отбрасывает варианты, нарушающие жёсткие ограничения;
6. ранжирует оставшиеся варианты;
7. формирует объяснимое решение или честно отказывается от рекомендации.
...
Система является советчиком. Она не записывает уставки в АСУ ТП и не управляет оборудованием напрямую.
...
| ЛИМС | Редкие лабораторные измерения | Обучающая цель и отложенный факт для shadow-проверки |
| ВАК | Расчётные показатели по инженерным формулам | Исследовательский и вспомогательный источник; не заменяет лабораторную цель |
...
Справочник подтверждает физический смысл тегов, но не подтверждает право изменять уставки, промышленные min/max или безопасный шаг воздействия.
...
- для полноценного прогноза требуется до шести часов истории.

На раскрытом test-периоде:
...
Модель H пока не превзошла исправный ПАК по MAE, поэтому production champion не назначен. Текущий ПАК используется как консервативная точечная оценка, а ML — как оценка риска, интервала и fallback.

Для кандидатов `T6/F9/P13` применяется отдельная прозрачная линейная what-if модель с горизонтом 0–3 часа. Её коэффициенты изолированы от CatBoost и обозначены как инженерные предположения, а не обученный причинный эффект.
...
В исходных данных нет журнала отказов оборудования. Поэтому `risk_score` — **индекс исторического отклонения**, а не вероятность аварии, отказа или оценка остаточного ресурса. Редкий режим не обязательно опасен, а частый режим не доказанно безопасен.

Оценка кандидата сейчас является статической подстановкой изменённых значений. Она не моделирует динамический отклик оборудования после управления. Подтверждённые технологические пределы и whitelist управляющих тегов в policy не заполнены, поэтому ReliabilityAgent не разрешает операторское изменение самостоятельно.

## Optimizer и Safety

Candidate Generator строит `NO_CHANGE` и ограниченную сетку изменений. Настроены шесть сценарных управлений:
...
За один запрос рассматривается не более 100 вариантов и не более двух одновременно изменяемых параметров. Диапазоны и шаги имеют статус `EXPERIMENTAL_MODEL`, а не утверждённого технологического регламента.
...
Безопасные варианты ранжируются лексикографически: вероятность нарушения качества → режимный риск → относительная стоимость → производственный эффект → размер действия. Полный экономический callback в текущей интеграции не подключён, поэтому система не заявляет максимизацию прибыли или выпуска.
...
| --- | --- |
| `GET /api/v1/health` | Состояние моделей, режимы адаптеров и production blockers |
...
1. `integration/examples/historical_20260515_1000.json` — исторический срез без подтверждённой рецептуры. Ожидаемый итог: `REFUSE`, потому что товарное качество нельзя достоверно собрать без плана смешения.
2. `integration/examples/illustrative_blend_20260515_1200.json` — измеренная телеметрия и ПАК плюс явно предположенная смесь 80/20. Ожидаемый итог: `KEEP`, прогноз серы около 7,27 мг/кг и лучший вариант `NO_CHANGE`.

Второй сценарий подтверждает работоспособность программного контура, но не является заводской рецептурой. Сохранённые примеры `RECOMMEND` в интерфейсе проверяют контракт и отображение; текущими моделями они не рассчитаны.
...
Тесты проверяют контракты, временную корректность признаков, отсутствие подстановки нулей, fallback-маршруты, генерацию кандидатов, Safety, решения Orchestrator и сквозной HTTP-вызов. Они подтверждают программное поведение, но не заменяют промышленную валидацию технологии.
...
│   ├── Quality_neftecode/    ← витрины и неактивные эксперименты
...
Полные CSV/XLSX и исследовательские артефакты не нужны для запуска готового API: активные model bundles и демонстрационные запросы уже находятся в `project`. Каталог `research` нужен для аудита, повторной подготовки данных и продолжения экспериментов.

## Исследовательские выводы

- ЛИМС остаётся эталонным, но редким и запаздывающим источником качества.
- Исправный ПАК на позднем test-периоде оказался сильным baseline; ML полезнее как оценка риска, интервала и fallback.
- Один глобальный сдвиг признаков на 60 или 120 минут ухудшил качество; рабочая схема использует временные окна и past-only признаки.
- В Reliability отсутствуют метки отказов, поэтому корректная задача — мониторинг исторического отклонения и достоверности данных.
- Поздний период показывает distribution shift: доля HIGH-сигналов Reliability заметно выросла относительно validation. Это требует анализа режима и перекалибровки, а не объявления всех сигналов авариями.
- Оптимизация качества без фактической рецептуры, запасов компонентов и подтверждённых управлений не может считаться производственной оптимизацией.
- Защитный отказ — полноценный результат системы, когда доказательств для рекомендации недостаточно.

## Ограничения текущей версии

Система является исследовательским прототипом и не предназначена для автоматического управления установкой.

- диапазоны и шаги `T6/F9/P13` не утверждены технологом;
- линейная candidate-модель не доказывает причинный эффект изменения уставки;
- смесь 80/20 и свойства второго компонента в демонстрации являются допущением;
- отсутствуют метки отказов оборудования и подтверждённые физические пределы Reliability;
- нет нового независимого holdout после раскрытого test-периода;
- экономический эффект и ограничения запасов не подключены;
- маршрут загрузки файлов требует дополнительного аудита происхождения и возраста измерений перед практическим использованием;
- API не имеет аутентификации и рассчитан на локальную демонстрацию;
- Dockerfile подготовлен, но контейнерная сборка в этой поставке отдельно не подтверждалась.

До снятия этих ограничений корректная интерпретация результата — совет в shadow-режиме с человеком в контуре.
...
- [Аудит рецептуры и управлений](project/neftecode_optimizer/docs/recipe_and_controls_audit.md)
